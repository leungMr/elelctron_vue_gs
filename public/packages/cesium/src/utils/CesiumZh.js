...
function CesiumZh() {
  ...
  function loadToolBarTitle() {
    //搜索
    updateAttribute(document, getChineseMaps(), "cesium-geocoder-input", "placeholder");
    //模式切换鼠标滑过提示
    updateAttributeByEvent(document, getChineseMaps(), "cesium-sceneModePicker-button3D", "title", "onmouseover");
    updateAttribute(document, getChineseMaps(), "cesium-sceneModePicker-dropDown-icon", "title");
    //初始视图
    updateAttribute(document, getChineseMaps(), "cesium-home-button", "title");
    //帮助
    updateAttribute(document, getChineseMaps(), "cesium-navigation-help-button", "title");
  }
  ...
  function loadBaseLayerPickerOfToolbar() {
    //获取容器
    let dropDownContainer = document.getElementsByClassName("cesium-baseLayerPicker-dropDown");
    if (dropDownContainer.length > 0) {
      dropDownContainer = dropDownContainer[0];
      //修改标题
      updateHtml(dropDownContainer, getChineseMaps(), "cesium-baseLayerPicker-sectionTitle");
      //副标题
      updateHtml(dropDownContainer, getChineseMaps(), "cesium-baseLayerPicker-categoryTitle");
      //具体标签
      updateHtml(dropDownContainer, getChineseMaps(), "cesium-baseLayerPicker-itemLabel");
      //具体地图描述
      updateAttribute(dropDownContainer, getChineseMaps(), "cesium-baseLayerPicker-item", "title");

      //鼠标悬浮事件监听，解决选择不同影像title 动态修改无法固定汉化问题
      let selectLayerToolbar = document.getElementsByClassName("cesium-baseLayerPicker-selected");
      if (selectLayerToolbar.length > 0) {
        selectLayerToolbar[0].parentElement.onmouseover = function (event) {
          ...
          let tempTitle1 = "";
          let tempTitle2 = "";
          if (getChineseMaps()[currentTargets[0]])
            tempTitle1 += getChineseMaps()[currentTargets[0]];
          else
            tempTitle1 += currentTargets[0];

          if (getChineseMaps()[currentTargets[0]])
            tempTitle2 += getChineseMaps()[currentTargets[1]];
          else
  ...
  function loadToolBarHelp() {
    //鼠标
    let mouseButton = document.getElementsByClassName("cesium-navigation-button-left");
    if (mouseButton.length > 0) {
      mouseButton = mouseButton[0];
      if (getChineseMaps()[mouseButton.textContent])
        getChineseMaps().innerHTML = mouseButton.children[0].outerHTML + getChineseMaps()[mouseButton.textContent];
    }
    let touchButton = document.getElementsByClassName("cesium-navigation-button-right");
    if (touchButton.length > 0) {
      touchButton = touchButton[0];
      if (getChineseMaps()[touchButton.textContent])
        touchButton.innerHTML = touchButton.children[0].outerHTML + getChineseMaps()[touchButton.textContent];
    }

    updateHtml(document, getChineseMaps(), "cesium-navigation-help-pan");
    updateHtml(document, getChineseMaps(), "cesium-navigation-help-details");
    updateHtml(document, getChineseMaps(), "cesium-navigation-help-zoom");
    updateHtml(document, getChineseMaps(), "cesium-navigation-help-rotate");
    updateHtml(document, getChineseMaps(), "cesium-navigation-help-tilt");
  }
  ...
  function loadFullExtent() {
    updateAttributeByEvent(document, getChineseMaps(), "cesium-fullscreenButton", "title", "onmouseover");
  }
  ...
  function loadFlyController() {
    let parentContainer = document.getElementsByClassName("cesium-viewer-animationContainer");
    if (parentContainer.length > 0) {
      parentContainer = parentContainer[0];
      updateHtmlByTagName(parentContainer, getChineseMaps(), "title");

      let datedes = parentContainer.getElementsByClassName("cesium-animation-svgText");
      for (let i = 0; i < datedes.length; i++) {
        let text = datedes[i].children[0].innerHTML;
        let texts = text.split(" ");
        if (texts.length === 3) {
          datedes[i].children[0].innerHTML = texts[2] + "年" + getChineseMaps()[texts[0]] + texts[1] + "日";
    ...
    //修改底部描述
    let cesiumDescription = document.getElementsByClassName("cesium-credit-textContainer");
    if (cesiumDescription.length > 0) {
      let insertEvent = function (e) {
        ...
        if (cesiumDescription[0].children.length > 0 && cesiumDescription[0].children[0].children.length > 0)
          cesiumDescription[0].children[0].children[0].innerHTML = getChineseMaps()["cesium_description"];
      };
      cesiumDescription[0].addEventListener('DOMNodeInserted', insertEvent);
    }

    //数据归属
    updateHtml(document, getChineseMaps(), "cesium-credit-expand-link");
    ...
    if (timeLineBar.length > 0) {
      ...
      timeLineBar.addEventListener('DOMNodeInserted', function (e) {
        ...
        if (e.target.getAttribute("class") === "cesium-timeline-ticLabel") {
          ...
          if (texts.length > 3) {
            e.target.innerHTML = texts[2] + "年" + getChineseMaps()[texts[0]] + texts[1] + "日 " + texts[3] + "";
          }
        }
      });

      for (let l of timeLineBar.getElementsByClassName("cesium-timeline-ticLabel")) {
        let texts = l.innerHTML.split(" ");
        if (texts.length > 3) {
          l.innerHTML = texts[2] + "年" + getChineseMaps()[texts[0]] + texts[1] + "日 " + texts[3] + "";
        }
      }
    }
  }
  ...
  function updateHtml(parent, map, className) {
    ...
    for (let dom of parent.getElementsByClassName(className)) {
      if (map[dom.innerHTML])
        dom.innerHTML = map[dom.innerHTML];
    }
  }
  ...
  function updateHtmlByTagName(parent, map, tagName) {
    ...
    for (let dom of parent.getElementsByTagName(tagName)) {
      if (map[dom.innerHTML])
        dom[i].innerHTML = map[dom.innerHTML];
    }
  }
  ...
  function updateAttribute(parent, map, className, attributeName) {
    ...
    for (let dom of parent.getElementsByClassName(className)) {
      if (map[dom.getAttribute(attributeName)])
        dom.setAttribute(attributeName, map[dom.getAttribute(attributeName)]);
    }
  }
